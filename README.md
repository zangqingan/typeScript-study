...
它们的类型约束(类型注解)在 TS 中都一一对应。

在 TypeScript 中,类型的标注主要通过类型后置语法来实现。也就是使用冒号后跟类型名来指定一个变量的类型。即 变量名:指定变量的类型,冒号的前后有没有空格都可以。
...
同时需要注意的是 TypeScript 的类型系统在很多情况下即使省略了类型注解也能自动推断出类型，这是 ts 的一个特性:即类型推论。
...
变量声明时,如果没有初始化且没有指定其类型,那么会被识别为 any 类型。any is hell ，尽量不要使用。
...
初始化并赋值了的变量、有默认值的函数形参、函数return返回的类型这3中都能被推断出来。
...
**注意：** 这个类型它还可以可以是接口或者类型别名,这样就变成了一个对象数组。
...
元组类型（ Tuple ）表示一个已知长度和类型的数组 其实可以理解为是一种特殊的数组。它可以指定数组元素的个数和类型。区别在于：数组合并了相同类型的对象,而元组（Tuple）合并了不同类型的对象。
...
**注意：** 如果函数没有明确返回值,默认返回 void 类型。JavaScript里返回undefined
...
也就是类型参数化，可以理解为类型的形参,通过类型变量将类型参数化。而类型变量是一种特殊的变量,只用于表示类型而不是具体的类型值。也就是将来使用具体的类型代替类型变量,一般可以用 T 代表 Type 来表示类型变量。
...
<T>,这样就定义了一个泛型。注意类型变量名可以随便取，但是一般使用常见约定的即可。
...
泛型最常用的场景就是用来约束函数参数的类型，我们知道之前 TS 约束函数是如下声明的,而泛型处理函数是在函数名后面加上泛型定义 <泛型变量名>,这样函数形参、函数返回值就可以使用泛型变量约束。这时在函数调用时再具体声明泛型变量的类型即可。
...
//类实现接口 implement
class IdentityClass<T> implement GenericInterface<T> {
value: T
constructor(value: T) {
this.value = value
}
